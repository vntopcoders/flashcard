...
CREATE INDEX IF NOT EXISTS idx_user_study_sessions_date ON next_auth.user_study_sessions(session_start::date);
...
-- Enable if you want additional security
-- =====================================================

-- Enable RLS on tables
-- ALTER TABLE next_auth.user_preferences ENABLE ROW LEVEL SECURITY;
-- ALTER TABLE next_auth.user_study_sessions ENABLE ROW LEVEL SECURITY;

-- Create policies (users can only access their own data)
-- CREATE POLICY "Users can view own preferences" ON next_auth.user_preferences
--   FOR SELECT USING (auth.uid() = user_id);

-- CREATE POLICY "Users can update own preferences" ON next_auth.user_preferences
...
-- LEFT JOIN next_auth.user_preferences p ON u.id = p.user_id
...
--   AVG(correct_answers::FLOAT / NULLIF(total_answers, 0)) as avg_accuracy
-- FROM next_auth.user_study_sessions
